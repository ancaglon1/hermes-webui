#!/usr/bin/env python3
"""Generate with all 4 Flux-2 Klein models + snofs LoKr."""
...
MODELS = [
    ("flux-2-klein-base-4b.safetensors", "qwen_3_4b.safetensors", "base4b"),
    ("flux-2-klein-4b.safetensors", "qwen_3_4b.safetensors", "4b"),
    ("flux-2-klein-base-9b-fp8.safetensors", "qwen_3_8b_fp8mixed.safetensors", "base9b"),
    ("flux-2-klein-9b-kv-fp8.safetensors", "qwen_3_8b_fp8mixed.safetensors", "9b_kv"),
]

RESULTS = []

for model_name, clip_name, label in MODELS:
    seed = random.randint(0, 2**32 - 1)
    print(f"\n=== {label} (model: {model_name}, clip: {clip_name}) ===", flush=True)
    ...
    n["4"] = {"class_type": "LoraLoader", "inputs": {"model": ["1", 0], "clip": ["2", 0], "lora_name": LORA, "strength_model": 0.8, "strength_clip": 0.8}}
    ...
    n["10"] = {"class_type": "SaveImage", "inputs": {"images": ["9", 0], "filename_prefix": f"flux2_{label}_snofs"}}
        ...
        try:
            ...
            if pid in hist:
                ...
                if s == "success" and imgs:
                    ...
                    RESULTS.append((label, path, f"seed={seed}"))
                    success = True
                elif s == "error":
                    for m in hist[pid].get("status",{}).get("messages",[]):
                        if m[0] == "execution_error":
                            err = m[1].get("exception_message","")[:200]
                            print(f"ERROR: {err}", flush=True)
                            RESULTS.append((label, None, err))
                            success = True  # Break outer loop
                if success: break
        except Exception as e:
            if i % 20 == 0: print(f"waiting ({i*5}s)", flush=True)
            time.sleep(5)
    if not success:
        RESULTS.append((label, None, "timeout"))

print("\n\n=== RESULTS ===", flush=True)
for label, path, note in RESULTS:
    status = f"OK: {path}" if path else f"FAIL: {note}"
