#!/usr/bin/env python3
"""Try 9B with different weight dtypes to bypass fp8 LoKr issue."""
...
# Try fp8_e4m3fn_fast (dequantize to fp16 on load)
for wd in ["fp8_e4m3fn_fast", "fp8_e4m3fn"]:
    seed = random.randint(0, 2**32 - 1)
    print(f"\n=== base9b with weight_dtype={wd} ===", flush=True)

    n = {}
    n["1"] = {"class_type": "UNETLoader", "inputs": {"unet_name": "flux-2-klein-base-9b-fp8.safetensors", "weight_dtype": wd}}
    n["2"] = {"class_type": "CLIPLoader", "inputs": {"clip_name": CLIP, "type": "flux2"}}
    n["3"] = {"class_type": "VAELoader", "inputs": {"vae_name": VAE}}
    n["4"] = {"class_type": "LycorisLoaderNode", "inputs": {"model": ["1", 0], "clip": ["2", 0], "lora_name": LORA, "strength_model": 0.8, "strength_clip": 0.8}}
    ...
    n["8"] = {"class_type": "KSampler", "inputs": {"model": ["4", 0], "seed": seed, "steps": 25, "cfg": 3.5, "sampler_name": "euler", "scheduler": "sgm_uniform", "positive": ["5", 0], "negative": ["6", 0], "latent_image": ["7", 0], "denoise": 1.0}}
    n["9"] = {"class_type": "VAEDecode", "inputs": {"samples": ["8", 0], "vae": ["3", 0]}}
    n["10"] = {"class_type": "SaveImage", "inputs": {"images": ["9", 0], "filename_prefix": f"flux2_base9b_{wd}"}}

    try:
        resp = urllib.request.urlopen(urllib.request.Request(f"{HOST}/prompt", data=json.dumps({"prompt": n}).encode(), headers={"Content-Type": "application/json"}, method="POST"), timeout=30)
