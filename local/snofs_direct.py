#!/usr/bin/env python3
"""Test SNOFS full models directly, no LoKr."""
...
PROMPT = "test image, elven warrior woman with sword, nude, erotic"
NEGATIVE = "blurry, low quality"

for model in ["snofsSexNudesAndOtherFunStuff_distilledV1.safetensors", "snofsSexNudesAndOtherFunStuff_v14Distilled.safetensors"]:
    seed = random.randint(0, 2**32 - 1)
    print(f"\n=== {model} ===", flush=True)
    n = {}
    ...
    n["9"] = {"class_type": "SaveImage", "inputs": {"images": ["8", 0], "filename_prefix": "snofs_test"}}
                    ...
                    if s == "success" and imgs:
                        ...
                        for m in hist[pid].get("status",{}).get("messages",[]):
                            if m[0] == "execution_error":
                                print(f"ERROR: {m[1].get('exception_message','')[:200]}", flush=True)
